...
typedef TSS2_RC (*yaml_parser_handler)(char *data, datum *value);
...
#define KVP_ADD_PARSER_SCALAR_U16(k, v, h) {.key = k, .value = { .type = data_type_p_y16, .as = { .p_y16 = { .sign = 0, .u = v, .handler=h}}}}
#define KVP_ADD_PARSER_SCALAR_U32(k, v, h) {.key = k, .value = { .type = data_type_p_y32, .as = { .p_y32 = { .sign = 0, .u = v, .handler=h}}}}
